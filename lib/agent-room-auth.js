...
function tokenHashes() {
  try {
    const parsed = JSON.parse(process.env.AGENT_ROOM_TOKEN_HASHES || "{}");
...
export function agentRoomActor(request) {
  ...
  for (const [actor, expected] of Object.entries(tokenHashes())) {
