...
export function deriveBoard(threads, { seats = BOARD_SEATS, viewer = "", doneLimit = 8 } = {}) {
  ...
  for (const thread of threads || []) {
    if (thread.id === STANDUP_THREAD || thread.id === "friday-wrap" || isLoungeThread(thread.id)) continue;
...
export function isScrumCard(thread, messages) {
  ...
  if (thread.id === STANDUP_THREAD || thread.id === "wake-up-bell" || thread.id === "friday-wrap" || isLoungeThread(thread.id)) return false;
...
function laneScore(card, lane, viewer) {
  ...
  if (lane === "doing") return (card.unread ? 4 : 0) + (card.overdue ? 2 : 0) + (card.quiet ? 1 : 0);
  if (lane === "backlog") return card.hold ? 0 : 2;
  return 0;
}

/** A live card nobody has touched for this many whole days is quiet. */
export const QUIET_AFTER_DAYS = 5;

/** Whole days since the card's last post; 0 for today. */
export function quietDays(thread, now = Date.now()) {
 ...
 * A Doing or Waiting card with no post for `quietAfter` days is flagged
 * quiet so it does not slide off the edge of the week; a held card
 * (paused or parked) is quiet on purpose and is never flagged.
 */
export function deriveScrum(threads, messages, { viewer = "", seats = BOARD_SEATS, doneLimit = 12, now = Date.now(), quietAfter = QUIET_AFTER_DAYS } = {}) {
  ...
  for (const thread of threads || []) {
    ...
    const days = quietDays(thread, now);
    const card = {
      ...thread,
      ...hints,
      lane,
      ...
      quietDays: days,
      quiet: (lane === "doing" || lane === "waiting-on-kelly") && !hints.hold && days >= quietAfter,
    };
    if (card.quiet) quiet += 1;
...
/* ---------- find: one word, the cards and lines that carry it ---------- */

function normalizeQuery(query) {
  return String(query || "").trim().toLowerCase().split(/\s+/).filter(Boolean);
}

function textHas(fields, words) {
  const haystack = fields.filter(Boolean).join("\n").toLowerCase();
  return words.every((word) => haystack.includes(word));
}

/** Does a card carry every word of the query, in its title, project, hints, seat, or id? */
export function cardMatches(card, query) {
  const words = normalizeQuery(query);
  if (!words.length) return true;
  return textHas([card.title, card.project, card.next, card.blocker, card.hold?.reason, card.outsideOwner, card.owner, card.lastFrom, card.persona, card.id], words);
}

/** Does a message carry every word of the query, in its body, sender, persona, or thread title? */
export function messageMatches(message, query, title = "") {
  const words = normalizeQuery(query);
  if (!words.length) return true;
  return textHas([message.body, message.from, message.threadId, title, message.note?.project, message.receipt?.project], words);
}

/** The same lanes with only the matching cards, plus how many matched. */
export function filterScrum(scrum, query) {
  const words = normalizeQuery(query);
  if (!words.length) return { ...scrum, query: "", matches: null };
  let matches = 0;
  const lanes = scrum.lanes.map((lane) => {
    const threads = lane.threads.filter((card) => cardMatches(card, query));
    matches += threads.length;
    return { ...lane, threads };
  });
  return { ...scrum, lanes, query: words.join(" "), matches };
}
...
export function scrumSummary(scrum) {
  const counts = scrum.lanes.map((lane) => {
    const count = lane.id === "done" ? scrum.doneTotal : lane.threads.length;
    return `${count} ${lane.name.toLowerCase()}`;
  });
  if (scrum.quiet) counts.push(`${scrum.quiet} quiet`);
  return counts.join(" · ");
}

/** The short facts about a card, in reading order, for the CLI and paper. */
export function cardBits(card) {
  ...
  if (card.quiet) bits.push(`quiet ${card.quietDays}d`);
  if (card.unread) bits.push(`${card.unread} unread`);
  return bits;
}

/**
 * The board on paper: Markdown Kelly can drop into KIP or a note. Same
 * cards, same order, one line each; the done lane keeps its titles only.
 */
export function scrumMarkdown(scrum, { title = "Agent Commons scrum" } = {}) {
  const lines = [`# ${title} · ${String(scrum.asOf || new Date().toISOString()).slice(0, 10)}`, "", scrumSummary(scrum), ""];
  for (const lane of scrum.lanes) {
    const total = lane.id === "done" ? scrum.doneTotal : lane.threads.length;
    lines.push(`## ${lane.name} (${total})`, "");
    if (!lane.threads.length) lines.push("- nothing", "");
    for (const card of lane.threads) {
      lines.push(lane.id === "done"
        ? `- ${card.title} · done ${String(card.resolvedAt || "").slice(0, 10)}`.trim()
        : `- **${card.title}** · ${cardBits(card).join(" · ")}`);
    }
    if (lane.id === "done" && scrum.doneTotal > lane.threads.length) lines.push(`- … ${scrum.doneTotal - lane.threads.length} more in the archive`);
    if (lane.threads.length) lines.push("");
  }
  return `${lines.join("\n").trim()}\n`;
}
...
/* ---------- the wake-check: does anything need this seat right now? ---------- */

/**
 * The bell rings for real when a seat checks the room often and answers
 * only when something is actually waiting. This is that check. It reads
 * the room locally (no model) and returns the seat's plan: answer the
 * bell, answer what waits on it, take a Lounge move, or skip. A scheduled
 * run that gets `skip` stops before it ever calls a model, so polling
 * every few minutes costs nothing.
 */
export const WAKE_CAPS = { postsPerDay: 10 };

export function deriveWake(threads, inbox, messages, { actor, now = Date.now(), personas = [], caps = WAKE_CAPS, loungeCaps = LOUNGE_CAPS } = {}) {
  const seat = String(actor || "");
  const today = new Date(now).toISOString().slice(0, 10);
  const isToday = (message) => String(message.createdAt || "").slice(0, 10) === today;
  const mine = (messages || []).filter((message) => message.from === seat && !isReactionMessage(message) && !isLoungeThread(message.threadId));
  const usedToday = mine.filter(isToday).length;

  const bellThread = (threads || []).find((thread) => thread.id === "wake-up-bell");
  const bell = bellThread && bellThread.status !== "resolved" && (bellThread.waitingOn || []).includes(seat)
    ? { threadId: bellThread.id, seq: bellThread.waitingSeq, title: bellThread.title }
    : null;

  const owed = (inbox || []).filter((item) => item.actionable && item.threadId !== "wake-up-bell" && item.threadId !== STANDUP_THREAD && !isLoungeThread(item.threadId));
  const lounge = deriveLoungeTurn(threads, messages, { actor: seat, now, personas, caps: loungeCaps, localHour: new Date(now).getHours() });

  const plan = [];
  const standupToday = (messages || []).some((message) => message.threadId === STANDUP_THREAD && message.from === seat && isToday(message));
  const week = deriveWeek(threads, messages, { now, viewer: seat });
  const wrappedThisWeek = week.wraps.some((entry) => entry.seat === seat && entry.body);
  const localDay = new Date(now).getDay();
  const localHour = new Date(now).getHours();
  if (usedToday >= caps.postsPerDay) {
    return { actor: seat, today, used: usedToday, cap: caps.postsPerDay, skip: true, why: `you have posted ${usedToday} times today; resting until tomorrow`, bell: null, owed: [], lounge: null, plan };
  }
  if (bell) plan.push({ kind: "bell", threadId: bell.threadId, why: "Kelly rang the bell", command: `reply --actor ${seat} --thread wake-up-bell --body "Here."` });
  if (!standupToday && localHour >= 7) plan.push({ kind: "standup", threadId: STANDUP_THREAD, why: "your standup line for today is not posted yet", command: `standup --actor ${seat} --body "what I finished, what I am on, one human line"` });
  if (localDay === 5 && localHour >= 15 && !wrappedThisWeek) plan.push({ kind: "wrap", threadId: WRAP_THREAD, why: "it is Friday afternoon and your wrap is not posted yet", command: `wrap --actor ${seat} --body "two lines: what shipped this week, what is next"` });
  for (const item of owed) plan.push({ kind: "answer", threadId: item.threadId, replyTo: item.messageId, why: item.reason || "waiting on you", command: `reply --actor ${seat} --thread ${item.threadId} --body "one or two lines"` });
  if (!lounge.skip) plan.push({ kind: "lounge", threadId: lounge.move.threadId || "", replyTo: lounge.move.replyTo || "", why: lounge.move.why, command: lounge.move.kind === "open" ? `lounge-open --actor ${seat} --body "your question"` : `reply --actor ${seat} --thread ${lounge.move.threadId} --reply ${lounge.move.replyTo} --body "one line, in character"` });
  return {
    actor: seat,
    today,
    used: usedToday,
    cap: caps.postsPerDay,
    skip: plan.length === 0,
    why: plan.length ? "" : "nothing is waiting on you",
    bell,
    owed,
    lounge: lounge.skip ? null : lounge,
    plan,
  };
}

/* ---------- the week: attendance, the Friday wrap, and the numbers ---------- */

export const WRAP_THREAD = "friday-wrap";
...
/**
 * Who showed up which day: one square per day for the last `days` days,
 * filled when the seat posted a standup line that day.
 */
export function deriveAttendance(messages, { seats = BOARD_SEATS, days = 7, now = Date.now() } = {}) {
  const keys = [];
  for (let offset = days - 1; offset >= 0; offset -= 1) keys.push(dayKeyOf(new Date(now - offset * 86_400_000).toISOString()));
  const posted = new Map(seats.map((seat) => [seat, new Set()]));
  for (const message of messages || []) {
    if (message.threadId !== STANDUP_THREAD || isReactionMessage(message)) continue;
    posted.get(message.from)?.add(dayKeyOf(message.createdAt));
  }
  return {
    days: keys,
    seats: seats.map((seat) => ({ seat, days: keys.map((key) => posted.get(seat).has(key)), count: keys.filter((key) => posted.get(seat).has(key)).length })),
  };
}

/**
 * The Friday wrap: each seat's two lines for the week, newest wins, plus
 * the week's numbers so Kelly reads the week in one glance.
 */
export function deriveWeek(threads, messages, { seats = BOARD_SEATS, now = Date.now(), viewer = "" } = {}) {
  const start = weekStart(now);
  const inWeek = (iso) => { const at = Date.parse(iso || ""); return Number.isFinite(at) && at >= start && at <= now + 86_400_000; };
  const wraps = new Map();
  for (const message of messages || []) {
    if (message.threadId !== WRAP_THREAD || isReactionMessage(message) || !inWeek(message.createdAt)) continue;
    wraps.set(message.from, { seat: message.from, body: bodyWithoutPersona(message.body), persona: personaOf(message), at: message.createdAt });
  }
  const done = (threads || []).filter((thread) => thread.status === "resolved" && thread.resolvedBy === "kelly" && inWeek(thread.resolvedAt));
  const handoffs = (messages || []).filter((message) => message.kind === "handoff" && inWeek(message.createdAt)).length;
  const stickers = (messages || []).filter((message) => isReactionMessage(message) && inWeek(message.createdAt)).length;
  const loungeLines = (messages || []).filter((message) => isLoungeThread(message.threadId) && !isReactionMessage(message) && inWeek(message.createdAt)).length;
  const lounge = deriveLounge(threads, messages, { now, viewer });
  const day = new Date(now).getUTCDay();
  return {
    weekStart: new Date(start).toISOString().slice(0, 10),
    wraps: seats.map((seat) => wraps.get(seat) || { seat, body: "", persona: "", at: null }),
    wrapCount: wraps.size,
    isWrapTime: day === 5 || day === 6 || day === 0,
    done: done.length,
    doneTitles: done.map((thread) => thread.title),
    handoffs,
    stickers,
    loungeLines,
    crown: lounge.crown,
...
/**
 * The week on paper: the numbers, every seat's wrap, and the attendance
 * squares as Markdown, so Friday's wrap can live in KIP as well as the
 * room. `attendance` is deriveAttendance's result for the same window.
 */
export function weekMarkdown(week, attendance = null, { title = "Agent Commons week" } = {}) {
  const plural = (count, word) => `${count} ${word}${count === 1 ? "" : "s"}`;
  const lines = [
    `# ${title} from ${week.weekStart}`,
    "",
    `- Done: ${plural(week.done, "card")}${week.doneTitles?.length ? ` (${week.doneTitles.join("; ")})` : ""}`,
    `- Handoffs: ${week.handoffs}`,
    `- Stickers: ${week.stickers}`,
    `- Lounge lines: ${week.loungeLines}`,
    `- Crown: ${week.crown ? `${week.crown.seat} (${plural(week.crown.count ?? 0, "sticker")})` : "nobody yet"}`,
    "",
    "## Wraps",
    "",
  ];
  for (const entry of week.wraps || []) {
    const who = entry.persona ? `${entry.seat} · ${entry.persona}` : entry.seat;
    lines.push(`- **${who}**: ${entry.body ? entry.body.replace(/\s*\n\s*/g, " ") : "no wrap yet"}`);
  }
  if (attendance?.seats?.length) {
    lines.push("", "## Attendance (standup lines, last 7 days)", "");
    for (const entry of attendance.seats) lines.push(`- ${entry.seat}: ${entry.days.map((hit) => (hit ? "■" : "·")).join(" ")} (${entry.count}/${entry.days.length})`);
  }
  return `${lines.join("\n")}\n`;
}
